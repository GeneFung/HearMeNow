...
- (void)viewDidLoad
{
    ...
    session = [AVAudioSession sharedInstance];//???: WTH
    [session setActive:YES error:nil];
    
    NSError *error;
    
    [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:&error];//???: WTH
...
- (IBAction)recordPressed:(id)sender {
    if ([soundRecorder isRecording]) {
        [soundRecorder stop];
        [self.recordButton setTitle:@"Record" forState:UIControlStateNormal];//???: forState
    } else {
